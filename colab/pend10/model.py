...
from pend10.swin_cross import SwinTransformer
...
class Fusion(nn.Module):
    def __init__(self, in_ch, embed_dim, shape, swin_depth,
        ...
        self.rgb_to_dep = SwinTransformer(
            ...
            attn_drop_rate=attn_dropout, cross_attention=True, ape=True
        )
...
class BBSNetTransformerAttention(BaseModel):
    def __init__(self):
        ...
        self.fuse0 = Fusion(
            in_ch=C[0], embed_dim=embed_dim[0], shape=S[0], swin_depth=4,
            num_heads=heads_stage[0], window_size=W[0], patch_size=P[0]
        )
        self.fuse1 = Fusion(
            in_ch=C[1], embed_dim=embed_dim[1], shape=S[1], swin_depth=4,
            num_heads=heads_stage[1], window_size=W[1], patch_size=P[1]
        )
        self.fuse2 = Fusion(
            in_ch=C[2], embed_dim=embed_dim[2], shape=S[2], swin_depth=4,
            num_heads=heads_stage[2], window_size=W[2], patch_size=P[2]
        )
        self.fuse3_1 = Fusion(
            in_ch=C[3], embed_dim=embed_dim[3], shape=S[3], swin_depth=4,
