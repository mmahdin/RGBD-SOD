...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True,
                 qk_scale=None, attn_drop=0., proj_drop=0., cross_attention=False
                 ):
        ...
        if self.cross_attention:
            # Separate linear layers for query, key, value
            self.q = nn.Linear(dim, dim, bias=qkv_bias)
            self.k = nn.Linear(dim, dim, bias=qkv_bias)
            self.v = nn.Linear(dim, dim, bias=qkv_bias)
        else:
            # Original combined qkv layer
    ...
    def forward(self, x, y=None, mask=None):
        ...
        """
        B_, N, C = x.shape
        if self.cross_attention and y is not None:
            # Cross-attention: query from x, key/value from y
            q = self.q(x).reshape(B_, N, self.num_heads, C //
                                  self.num_heads).permute(0, 2, 1, 3)
            k = self.k(y).reshape(B_, N, self.num_heads, C //
                                  self.num_heads).permute(0, 2, 1, 3)
            v = self.v(y).reshape(B_, N, self.num_heads, C //
                                  self.num_heads).permute(0, 2, 1, 3)
        else:
            # Self-attention
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, cross_attention=False):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
