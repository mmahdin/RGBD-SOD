...
class CoarseCrossAttention(nn.Module):
    ...
    def forward(self, x_q, x_kv):
        ...
        if x_q is x_kv:
            pooled = self.pool(x_q)
            q = self.q_proj(pooled)
            k = self.k_proj(pooled)
            v = self.v_proj(pooled)
        else:
            q = self.q_proj(self.pool(x_q))
            k = self.k_proj(self.pool(x_kv))
            v = self.v_proj(self.pool(x_kv))
        ...
        # 6) Project to embed_dim and upsample + residual
...
class Fusion(nn.Module):
    def __init__(self, in_ch, embed_dim, shape,
                 num_heads, mlp_ratio=4,
                 attn_dropout=0.2, dropout=0.1,
                 cross_patch=8, cross_heads=None,
                 cross_attn_dim=None,
                 cross_pool="avg"):
        ...
        # Replace Swin self-attention with CoarseCrossAttention (self-guided)
        self.rgb_self = CoarseCrossAttention(
            embed_dim=embed_dim,
        ...
        self.dep_self = CoarseCrossAttention(
            embed_dim=embed_dim,
            attn_dim=cross_attn_dim or embed_dim,
            num_heads=cross_heads or num_heads,
            patch=cross_patch,
        ...
        # I’d recommend GroupNorm over BatchNorm for your dataset
        self.out_norm = nn.GroupNorm(num_groups=8, num_channels=embed_dim)

    def forward(self, Ri, Ti):
        B, C, H, W = Ri.shape

        # 1) Coarse self-attention (just pass same tensor twice)
        rgb_self = self.rgb_self(Ri, Ri)
        dep_self = self.dep_self(Ti, Ti)
...
class BBSNetTransformerAttention(BaseModel):
    def __init__(self):
        ...
        # Coarse cross-attention patch sizes (larger = more aggressive pooling)
        cross_patch = [8, 8, 4, 2, 1]

        self.fuse0 = Fusion(
            in_ch=C[0], embed_dim=embed_dim[0], shape=S[0],
            num_heads=heads_stage[0], dropout=dropout,
            cross_patch=cross_patch[0], cross_pool="avg"
        )
        self.fuse1 = Fusion(
            in_ch=C[1], embed_dim=embed_dim[1], shape=S[1],
            num_heads=heads_stage[1], dropout=dropout,
            cross_patch=cross_patch[1], cross_pool="avg"
        )
        self.fuse2 = Fusion(
            in_ch=C[2], embed_dim=embed_dim[2], shape=S[2],
            num_heads=heads_stage[2], dropout=dropout,
            cross_patch=cross_patch[2], cross_pool="avg"
        )
        self.fuse3_1 = Fusion(
            in_ch=C[3], embed_dim=embed_dim[3], shape=S[3],
            num_heads=heads_stage[3], dropout=dropout,
            cross_patch=cross_patch[3], cross_pool="avg"
        )
        self.fuse4_1 = Fusion(
            in_ch=C[4], embed_dim=embed_dim[4], shape=S[4],
            num_heads=heads_stage[4], dropout=dropout,
            cross_patch=cross_patch[4], cross_pool="avg"
        )
