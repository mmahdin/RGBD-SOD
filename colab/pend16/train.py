...
from pend16.options import opt
import torch.backends.cudnn as cudnn
import logging
from tensorboardX import SummaryWriter
from pend16.model import BBSNetTransformerAttention as BBSNet
...
def train(train_loader, model, optimizer, epoch, save_path, pending_unembed=None):
    ...
    for i, (images, gts, depths) in enumerate(train_loader, start=1):
        ...
        loss2 = structure_loss(s3, gts)
        loss = 0.8*loss1 + loss2 + 0.6*loss2
        # loss1 = CE(s1, gts)
        # loss2 = CE(s2, gts)
        # loss = loss1 + loss2
        loss.backward()
