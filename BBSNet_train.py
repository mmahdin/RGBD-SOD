...
def train(train_loader, model, optimizer, epoch, save_path, CE, total_step):
    ...
    try:
        ...
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path +
                   'BBSNet_epoch_{}.pth'.format(epoch+1))
...
def test(test_loader, model, epoch, save_path):
    ...
    with torch.no_grad():
        ...
        print('Epoch: {} MAE: {} ####  bestMAE: {} bestEpoch: {}'.format(
...
def main(optimizer, model, train_loader, test_loader, CE, total_step):
    print("Start train...")
