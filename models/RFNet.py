...
class AF0(nn.Module):
    def __init__(self, indim, outdim):
        ...
        self.alpha_C = nn.parameter.Parameter(
            torch.Tensor(1), requires_grad=True)
        self.alpha_S = nn.parameter.Parameter(
            torch.Tensor(1), requires_grad=True)
...
class RFNet(nn.Module):
    ...
    def forward(self, imgs, depths):
        ...
        ful_2 = self.fu_2(
            img_2, dep_2, lambdaLWA[:, 2:3, ...], self.pool_fu_1(ful_1))
        ful_3 = self.fu_3(
            img_3, dep_3, lambdaLWA[:, 3:4, ...], self.pool_fu_2(ful_2))
        ful_4 = self.fu_4(
            img_4, dep_4, lambdaLWA[:, 4:5, ...], self.pool_fu_3(ful_3))
