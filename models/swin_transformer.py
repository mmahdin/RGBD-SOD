...
class SwinNet(nn.Module):
    ...
    def load_pre(self, pre_model):
        self.rgb_swin.load_state_dict(
            torch.load(pre_model)['model'], strict=False)
        print(f"RGB SwinTransformer loading pre_model ${pre_model}")
        self.depth_swin.load_state_dict(
            torch.load(pre_model)['model'], strict=False)
...
class RCAB(nn.Module):
    def __init__(
        ...
        modules_body = []
        for i in range(2):
            modules_body.append(self.default_conv(
...
class Edge_Module(nn.Module):
    ...
    def forward(self, x2, x4, x5):
        ...
        edge4 = F.interpolate(edge4, size=(
            h, w), mode='bilinear', align_corners=True)
        edge5 = F.interpolate(edge5, size=(
            h, w), mode='bilinear', align_corners=True)
