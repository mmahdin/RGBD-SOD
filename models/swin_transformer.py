...
class StageModule(nn.Module):
    def __init__(self, in_channels, hidden_dimension, layers, num_heads, head_dim, window_size,
        ...
        super().__init__()
        # Just a channel projection (optional if in_channels == hidden_dimension)
        self.proj = nn.Linear(
            in_channels, hidden_dimension) if in_channels != hidden_dimension else nn.Identity()
...
class SwinTransformer(nn.Module):
    def __init__(self, *, hidden_dim, layers, heads, channels, head_dim=32, window_size=7,
    ...
    def forward(self, x):
        return self.stage1(x)  # returns (B, hidden_dim, H, W)
