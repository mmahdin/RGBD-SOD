...
class lwa(nn.Module):
    def __init__(self, channel, outsize):
        ...
        self.mlp = nn.Sequential(_ConvBNReLU(
            channel, 24, 1, 1), _ConvBNSig(24, outsize, 1, 1))
    ...
    def forward(self, rgb, dep):
        ...
        proj_rgb = self.rgb(rgb).view(
            m_batchsize, -1, height*width).permute(0, 2, 1)  # B X (H*W) X C
        proj_dep = self.dept(dep).view(
            m_batchsize, -1, height*width)  # B X C x (H*W)
        energy = torch.bmm(proj_rgb, proj_dep) / \
            math.sqrt(C)  # B X (H*W) X (H*W)
