...
def main(cfg):
    ...
    for dataset in dataset_names:
        for method in method_names:
            loader = EvalDataset(
                osp.join(pred_dir, method, dataset), osp.join(gt_dir, dataset))
...
def draw_curve():
    ...
    for dataset in datasets:
        ...
        plt.legend(methods, loc='lower left',
                   fontsize=_font_size_ - 2, framealpha=0.75)
        plt.savefig(os.path.join(curve_save_dir,
                    '{}_pr.png'.format(dataset)), bbox_inches='tight')
        # plt.show()
        ...
        plt.legend(methods, loc='lower left',
                   fontsize=_font_size_ - 2, framealpha=0.75)
        plt.savefig(os.path.join(curve_save_dir,
                    '{}_fm.png'.format(dataset)), bbox_inches='tight')
